...
A normal map PNG has no field for which way green points, and the two conventions (Y+ "OpenGL", which Unity's manual says it uses, and Y− "DirectX") differ only in that sign. A map of the wrong kind imports without a word and every bump reads as a dent. **We shipped ten of those** in a free fabric pack on 2026-09-13 and found out four days later by reading source code, not pixels. [`normal-y-check/`](normal-y-check/) decides from the pixels: a real slope field has no curl, so the two mixed differences (across-slope going down, down-slope going across) agree under the right reading of green and come out equal and opposite under the wrong one. **Relief that bends only one way — a stripe, a flat map, noise — comes back `undecided` with the reason, never a guess.** On our own shelf: the 09-13 pack reads 10 Y− and 1 undecided (the stripe), the fixed pack 10 Y+ and 1 undecided, and 65 more maps from ten generators 64 Y+, 1 undecided, 0 wrong (64 Y− with green inverted). 35 tests, 21 deliberate mutations, all caught — **the first run caught 19**, and one survivor was a verdict coming out right for the wrong reason. **Every map it has seen is ours**; one baked in Substance or Blender would be worth more than that whole table. **No Python needed:** fork the repository, paste [`normal-y-check.yml`](normal-y-check/normal-y-check.yml) into the fork's `.github/workflows/`, then *Actions → normal-y-check → Run workflow* with the image's link; the verdict and a green-inverted copy of any map pointing the wrong way come back as an artifact.

### `forge-scope-check/` — two of our five Forge apps could not read a single board or group member, and every file looked fine

A Forge app's scopes live in `manifest.yml` and its Jira calls live in the code, and nothing checks one against the other until a real site answers `403`. Ours would have: a JQL function declared only `read:jira-work` for four Jira Software calls (Jira Software has no classic scopes), another called `group/member` without the scope it needs, and both called `api.asUser()` from a JQL function, which is not a UI module. [`forge-scope-check/`](forge-scope-check/) reads every `requestJira(route`...`)`, takes the method from the options, and looks the call up in a table of **724 operations** built from Atlassian's published OpenAPI definitions: all of the classic set or all of the granular set, never half. **A call it cannot resolve is `unchecked`, and exit 3 is kept apart from a pass.** On our own five apps before we fixed them by hand: 26 calls, findings in 3 apps (6 missing, 10 `asUser()` lines, 2 deprecated) — every one a bug we had found by hand; now 23 calls, 0. 41 tests; 21 deliberate mutations, **the first run caught 18**. **Every app it has seen is ours**; one run on someone else's Forge app would be worth more than that table. Standard library only, no Node.

### `einvoice-total-check/` — an invoice with 27 cents too little VAT passed the official EN 16931 rules, and that is by design

The CEN rules that EN 16931 e-invoices in Germany and the rest of the EU are validated against let a VAT amount be off by up to 1 in any currency (BR-S-09 `< 1`, BR-CO-17 `<= 1`); upstream [#432](https://github.com/ConnectingEurope/eInvoicing-EN16931/issues/432) is an invoice that said 13.22 where 13.49 was right, and passed. [`einvoice-total-check/`](einvoice-total-check/) recomputes ten arithmetic rules of a CII invoice (ZUGFeRD / Factur-X / XRechnung) with the standard library — no Java — names the declared amount that disagrees and what it should be, and reports what the slack lets through as a separate `STRICT-VAT` line. **Every rule is evaluated in decimal and in double and under two tie rules; a rule that does not come out the same is `SPLIT`, exit 3, not a pass.** The 15 official CII examples: 15 pass, 0 split. The same 15 with one amount changed at a time: every change caught by its rule; VAT +0.99 with the totals moved to match passed every official rule on 11 of 12, and `STRICT-VAT` flagged all 12. Our first run rejected a valid official example: we had printed a rule cut at 600 characters and missed its third branch. 40 tests; 20 mutations, **the first run caught 19**. **No invoice from a real generator has been through it yet.**
...
Nobody keeps the history of "ready, unassigned, not blocked", and Jira has no field for it. [`idle-day-forecast/`](idle-day-forecast/) rebuilds that count for every past day from each issue's changelog — blockers read through their own changelogs, issues moved in from other projects counted from the move — and says when it reaches 0 at the recent pace. **When the history cannot support a forecast (a cut-short changelog, a renamed status, a blocker you cannot read) it refuses, exit 2, and says why.** Checked against Jira's own `WAS ... ON` search on 130 days of 10 public Apache projects: **9,907 of 9,907 issue-days the same, 0 disagreements.** On Jira Cloud (Red Hat's public site) Jira was the side that was wrong: `status WAS "Open"` missed an issue Open since 2021 (by id it did not), `assignee WAS EMPTY` missed every issue unassigned since creation, and four issues closed in 2020–21 came back as open on every day we asked. 42 tests; 23 mutations, **the first run caught 21 of 22**. **The forecast itself has never met a real idle day**: none of the 11 queues we could read ran dry even once in a year, so its backtest has only scored "no idle day, and none came" — which "always say no" also gets. Standard library only.
...
`java.text.MessageFormat` reads an apostrophe as the start of a quoted section, so `O agente chamado '{0}' já existe` (Brazilian Portuguese, written for the base's typographic `‘{0}’`) prints the braces and no name, and `dev'essere` swallows the rest of an Italian sentence. The file loads, the English build is fine, nothing fails. [`i18n-placeholder-check/`](i18n-placeholder-check/) reads `.properties` bundles the way `Properties.load` and `MessageFormat.applyPattern` do and compares every translation with its base: a placeholder quoted away (`QUOTED`), a value that throws (`BROKEN`), an apostrophe that vanishes (`APOS`), placeholders dropped or added, a byte-order mark that breaks the first key. **Replaying Jenkins' own 12 apostrophe-fix commits (2008–2019): all 36 changed messages flagged before the fix, and the 2 still flagged after were left broken by that commit and fixed years later; its 3 style-only commits (96 messages) got 0.** On today's master (7,489 files): 11 quoted-away placeholders in 10 messages, 80 vanishing apostrophes, 1 value that throws. **Nothing here has been seen in a running Jenkins** — the parser follows the OpenJDK 21 and 25 source, read, not run. 55 tests; 26 mutations, **the first run caught 21 of 22**. Standard library only.

### `setup-doc-check/` — bat's README says Rust 1.79 builds it; its Cargo.toml refuses anything older than 1.88

A setup section goes stale one rename at a time, and nothing runs the README. [`setup-doc-check/`](setup-doc-check/) reads the shell commands in a repository's README, CONTRIBUTING and setup docs and asks the repository — not a shell — whether each can work: the npm script in the `package.json` it would reach, the Makefile rule, the file a `cd`, `cp`, `source`, `pip -r` or `./x` needs, the language version the prose names against `engines.node`, `requires-python`, `go.mod`, `rust-version`. Nothing is run. A block that does not `cd` is reported only when it is wrong read every way it could be (where the last block left off, the root, a folder the prose just named). **Replaying 27 commits whose message says they fixed a README's commands: 15 were flagged before and clean after, 1 the other way round (a README renamed ahead of its `package.json`), 11 not — 8 of those were not a name a repository can answer (a Gradle test, a Go module path, a `chmod` glob).** On 100 well-known repositories today it found, among others, bat, hyperfine and hexyl asking for older Rust than their `rust-version`, rich saying Python 3.8, luxon saying Node.js 6+ and `npm run docs` (the script is `api-docs`), and undici's `cd docs && npm run serve` reaching a root `package.json` that has no `serve`. **Held out before tuning, errors were 9 of 14 real and warnings 2 of 12**; after the fixes those prompted, 22 of 30 on all 100 — no longer a held-out number. 89 tests; 54 mutations, **the first run of the first 32 caught 24**. Standard library only.

### `migration-collision-check/` — each branch passes its own checks; the merge adds a second V24

Two branches each add the next database migration and pick the same number. Both are green; the merge is not, and the first place it shows is the base branch or the deploy. [`migration-collision-check/`](migration-collision-check/) reads the migration files of Flyway, Django, Alembic, Rails and golang-migrate and asks each set what its own tool asks before it runs: two files with one version, two leaf migrations in a Django app, two Alembic heads, a dependency on a migration that is not there. With `--base origin/main` it builds the tree the merge would give and reports only what the branch brings in — including a Flyway or golang-migrate migration numbered below one the base already has. **Replaying 106 public commits that fixed a collision, split in half before tuning: of the 53 held out, 46 were flagged before the fix and clean after, 1 more as a warning; half of the misses were renames made to get out of the way of the other branch — the case `--base` is for.** On the default branches of 99 well-known repositories, the held-out 49 gave 34 errors in 3 repositories and none was real (PostHog's own async migrations, Prefect's per-database chains, Harness's name-ordered migrator); after the fixes, 0 errors on all 99 (34,003 migration files). 88 tests; 62 mutations, **the first run of the first 42 caught 41**. The CI snippet in its README has not been run on GitHub by us. Standard library only.

### `changelog-delta-check/` — rack's changelog dates 2.2.9 four months before 2.2.8. RubyGems has it a year later

A wrong date in a changelog looks like any other date, and a compare link that starts one release too early opens a perfectly good diff. What gives each away is the entry next to it — which is how a clinical lab catches a mixed-up specimen: not by re-checking the result, but by comparing it with the patient's previous value (a *delta check*). [`changelog-delta-check/`](changelog-delta-check/) reads every release entry of a CHANGELOG / CHANGES / HISTORY / NEWS file — version, date, link — and compares each with its neighbours: a later version dated before the one below it in the same release line, a date one year off from fitting between its neighbours, an `[Unreleased]` link still comparing from an older release, a compare link that skips a release the file dates earlier, a heading linked to another version's tag. A date it cannot read one way only (`03/04/2024`) is counted, not guessed. **Replaying 313 public commits whose patch changed a release heading's date or a version's link, split in half before tuning: of the 156 held out, 46 were flagged before the fix and clean after** — links 36 of 66, dates 12 of 88. Most dates people fix fit between their neighbours (94 moved by two weeks or less), and the checker caught none of the 9 one-year fixes: every entry had moved together. **On 112 well-known changelogs, the held-out half's first run gave 97 errors and 13 were real** (Grafana's `+security-01` re-releases and Consul's Enterprise editions read as duplicates); after the fixes, **29 errors on all 112 and all 29 are real** — 17 checked against npm, PyPI, RubyGems, crates.io or the release page (rack 2.2.9, mdBook 0.2.3, requests 0.10.2, Textual's `[7.0.0]` skipping 6.12.0 …), 12 readable in the file (Flutter's 3.19.3 heading linked to the 3.16.3 release). 67 tests; 43 mutations, **the first run caught 39**. Standard library only.

### `table-total-check/` — Llama 2's model card lists three models and a total for four

The carbon table in `meta-llama/llama`'s model card has rows for 7B, 13B and 70B and a Total of 3,311,616 GPU hours; the rows make 2,273,280. The paper's table has a fourth row, 34B, at exactly the difference. Nobody re-adds a bold **Total** row. [`table-total-check/`](table-total-check/) does: in every Markdown table it re-adds the rows labelled Total / Subtotal / 合計 / 小計 …, the columns headed Total and the percentage columns under a total, compares whole numbers and money exactly and everything shown rounded within its own rounding, accepts a total that is a mean, a max or a weighted mean, and never guesses a cell it can read two ways (`1.429`, `~120`, `900 KB` next to `1.2 MB`, a `...` row). **On 2,787 Markdown files with "Total" under 30 well-known organisations, halved before tuning: the held-out half's first run gave 4 errors, 3 real; after tuning, 6 errors on all of them and all 6 are real** (the Llama 2 card and its copy in `microsoft/Llama-2-Onnx`, `airbnb/mavericks`' `mvp-clean` 2,385 for 3,385, `uber/tango`, a JetBrains timing table). Of 61 public commits that changed a table's rows and total together, the final version flags 5 and all 5 are real mistakes left in (`eshu-hq/eshu` 368 / 713 for 370 / 714 …). Only 6 of 4,566 "fix total" commits changed a total alone; it catches 2, flags 2 whose fixed total still does not add up, warns on 1 (blank rows) and misses 1 (the total cell is a sentence). 57 tests; 45 mutations, the first run caught 25 of 33. Standard library only.

### `label-link-check/` — Bootstrap's bug report form files every report under `bug`. Bootstrap has no label called `bug`

`twbs/bootstrap`'s bug form says `labels: [bug]` and its README's "Report bug" link says `labels=bug`; none of its 69 labels is `bug`. As GitHub's own page says, a label that does not exist is simply not added, and `label:bug` finds 0 issues. A link to a label that does not exist still answers HTTP 200, so link checkers pass it. [`label-link-check/`](label-link-check/) reads the links (`/labels/NAME`, `issues?q=label:…`, `issues/new?labels=…&template=…`) and the `labels:` of every issue template, checks them against the repository's real labels and template folder, and names what is missing with the closest real name — plus an unquoted `label:good first issue` and quotes inside a `/labels/` path. **On 486 well-known repositories (16,793 files), halved before tuning, the held-out half's first run gave 140 errors and 136 were real** (the other 4: our measurement had cut `elastic/kibana`'s label list at 1,000). **The final version reports 204 errors in 86 repositories, every one confirmed** — 62 of the 366 with issue templates name, between them, 100 labels that do not exist (open-webui's `triage`, langchain's `task`, llama.cpp's `refactor` …), and p5.js's contributor guide links, 30 times in six languages, to template names from before it numbered them. Of 164 public commits that fixed a label link or template labels, 36 of the 80 held out were flagged before and clean after, and the one real miss was a `!` at the end of a URL; Next.js #80478 "fixed" a working link into a broken one. GitHub's search also turns out to remember deleted labels (docusaurus' `help wanted` returns 186 issues, and none of the first 30 carries it). This repository's own issue templates named six labels that did not exist; that was the first finding, and it is fixed. 83 tests; 41 mutations, the first run caught 27 of 29. Standard library only.

### `moved-path-check/` — replicate/cog's AGENTS.md still sends coding agents to a file that was deleted in February

`replicate/cog`'s `AGENTS.md` lists `python/cog/base_predictor.py` among its "Important Files"; commit `df576ff` deleted it on 2026-02-04, and `AGENTS.md` was edited again in April without noticing. A path in backticks or in a shell block is not a link, so link checkers never read it, and grepping for paths that do not exist drowns in the reader's own files. [`moved-path-check/`](moved-path-check/) asks the repository's history instead: a path that never existed here is somebody else's; a path that existed until a commit removed it is a stale sentence — reported with the commit, the date and, when git knows, where the file is now. **The first version was bad: 38 of 200 sampled errors were real (19%)** — most of the rest were dated plans and the reader's `.env`. On 271 repositories no version had been run on, **the final version reports 198 errors in 42 of them, and 177 are real (89%)**; 19 are in files written for coding agents. Of 29 public commits that fixed a stale doc by hand, it catches 16 outright (the first version caught 19 — that is the price). 60 tests; mutations: 43, all caught (the first run caught 36 of 42). Standard library only.
...
A comment that says *"I'd like to work on this"* takes an issue off every `no:assignee` list once a maintainer assigns it, and nothing expires it: a stale bot counts days since any activity, and every *"can I take this?"* is activity. [`claim-check/`](claim-check/) reads each open issue's timeline and asks, for every person who claimed it, whether they have done anything on it since — a comment, a commit, a pull request that links or names the issue — and reports `GONE` (still assigned to them), `QUIET` (a maintainer answered, then nothing) and `UNANSWERED`. **The first thing we measured was wrong: with a fine-grained token not granted the repository, timelines return not one cross-reference** (41,780 issues, 0), so delivered work looked abandoned; closing pull requests from GraphQL removed 36% of the first version's `GONE`, and two searches per finding (the claimant's pull requests, and pull requests containing the issue number) drop a quarter to a third of the rest. On 333 repositories with good first issues, halved before tuning, **the first version's `GONE` was 51% real (95 of 185) on the held-out half; the final version's is 69% (103 of 150) on 83 repositories no version had seen**, and most of the rest are pull requests that do the work without naming the issue. Replayed from 2026-03-24, **1 in 5 claimants silent under 30 days came back within six months; past 30 days, 1 in 42.** Of 344 hand-written *"are you still working on this?"* comments, the checker had the claim open for half, and asked within 90 days the claimant answered within a month 70–78% of the time — past 90 days, 38%. 44 tests; 47 mutations, all caught (the first run caught 33 of 35). Standard library only.
