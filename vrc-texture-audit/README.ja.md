...
| 4 成分すべてを復元 | 2.75 秒 |
| **アルファ板だけを復元** | **0.90 秒** |
| 1,000 枚・ディスク上 2.2 GB・実測 1 回(下記) | **69.9 秒** |
...
- **1,000 枚級で実測は済んだ —— ただし合成データで、実際のアバター案件ではない。**1,000 ファイル・ディスク上 2.2 GB、寸法は 256〜4096 に散らした(PNG 850 枚の内訳は素の RGB 494・全画素不透明の RGBA 236・実際に抜きのある RGBA 120。残り 150 はヘッダだけ読む JPG/TGA/BMP/GIF)。実行は **69.9 秒**、物理メモリは **24 MB 未満**に収まった(Windows の `PeakWorkingSetSize`。500 MB を意図的に確保して数値が実際に跳ね上がることを確かめた上で採用)。1 ファイルずつ流して読む設計のため画像 1 枚分もメモリに載らず、時間は画素数に応じて伸びてもメモリは平らなままだった。実測 2026-09-15・このマシン・Python 3.13.3。**まだ埋まっていない分**: 実際に出荷されたアバター案件のフォルダには一度も向けていない(向けたのはこの合成データだけ)。別のマシン・遅いディスクでは所要時間の数字は動く。
