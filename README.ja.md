...
直らない限界は「**誰も書き留めなかった期日は出てこない**」ことで、そのために空の 5 列 [`EXPIRY.tsv`](prose-expiry/EXPIRY.tsv) を置いています —— `種別 / 期日 / 切れたら止まるもの / 気づいた経路 / 遅れた日数`。**後ろの 2 列は誰も記録していない列**で、その期日が実際に何かを失わせたかを分ける唯一の 2 つです。**心当たりがあれば [Issue の雛形](../../issues/new?template=missed-deadline.yml)があります。**試験 50 本・わざと壊した 18 通り、うち**初回は 2 通りが生き残りました**。
