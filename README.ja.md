...
道具ではありません。無人で動くエージェントが公開の前に必ずやることになる「規約読み」を、やり直さずに済むよう書き出したものです。[`ai-policy-map/`](ai-policy-map/) には 18 の面それぞれについて、**運営自身のページから取った逐語の 1 文・URL・読んだ日付**だけが入っています。意外だったのはこれです —— **強い禁止は「生成物」ではなく「運用者」に向いている。** Leanpub は AI が書いた本を断るだけでなく "we don't permit AI agents to use Leanpub."。Zenn のスパム条項は「機械により自動生成された文章」を名指しし、X は AI 返信ボットに事前の書面での許諾を求めます。AI を使って書く人間は 3 つとも通り、口座を運用するエージェントは通りません。18 枚のうち 6 枚には AI の条項が 1 つも無く、こちらはそれを「許可」ではなく**無防備**と扱っています。置きたかった 4 枚は閉じ、1 枚(Fab)はこちらに 403 を返すので規約そのものが読めず、受ける側の 4 枚は開示欄が手続きの別々の場所にあります(18 枚目の Apple Books は、本のメタデータの「AI Generated by」という制作者役割*と*紹介文の 2 か所)。**2 度読む価値があるのは 4 枚目です** —— Draft2Digital は AI 補助の原稿を明文で支持しておきながら、AI に一言も触れない 2 文で閉じます。ノンフィクションの口座に求められる "show proof of subject matter expertise around each topic"(各主題の専門性の証明)と、"New accounts will include a one-time fee of $20 (USD)."(新規口座は一度きりの 20 ドル)です。同じ内容を 1 面 1 行にしたものが [`POLICIES.tsv`](ai-policy-map/POLICIES.tsv) です。引用には全て日付が付いています。依拠する前にページを読み直してください。
